...
fn is_valid(grid: &Board, row: usize, col: usize, value: u8) -> bool {
    // is unique in row
    for v in 0..9 {
        if value == grid[(row, v)] {
            return false;
...
fn solve(grid: &mut Board, row: usize, col: usize) -> bool {
    if row == 9 {
        true
    } else if col == 9 {
        solve(grid, row + 1, 0)
...
/// Verify that we can solve the board by checking row/col/box collisions.
/// '0' values in the grid are ignored.
fn is_solvable_board(grid: &Board) -> bool {
    ...
    for row in 0..9 {
        ...
        for col in 0..9 {
            let row_value = grid[(row, col)];
            if row_value != 0 && !seen_row.insert(row_value) {
                return false;
            }
            let col_value = grid[(col, row)];
...
impl super::SudokuSolver for Board {
    fn solve(&self) -> Option<Solution> {
        ...
        if is_solvable_board(&grid) && solve(&mut grid, 0, 0) {
            Some(grid)
        } else {
            None
        }
