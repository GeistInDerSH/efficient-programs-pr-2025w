...
static int is_valid_optimized_v2(const struct Board* board, int row, int col, uint8_t value) {
    // is unique in row
    for (int v = 0; v < 9; v++) {
...
static int solve_recursive_optimized_v2(struct Board* board, int row, int col) {
    if (row == 9) {
        return 1; // true: All rows solved
    } else if (col == 9) {
        return solve_recursive_optimized_v2(board, row + 1, 0); // Move to next row
    } else if (board->cells[row * 9 + col] != 0) {
        return solve_recursive_optimized_v2(board, row, col + 1); // Skip filled cell
    } else {
        // Try all numbers
        for (uint8_t p = 1; p <= 9; p++) {
            if (!is_valid_optimized_v2(board, row, col, p)) {
                continue;
            }
            board->cells[row * 9 + col] = p; // Make guess
            if (solve_recursive_optimized_v2(board, row, col + 1)) {
...
int solve_optimized_v2( struct Board* input, struct Board* solution) {
    ...
    // printf("solve_optimized_v2\n");

    memcpy(solution, input, sizeof(struct Board));

    return solve_recursive_optimized_v2(solution, 0, 0);
}